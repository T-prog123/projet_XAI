...
class SparsifySSTWrapper(nn.Module):
    ...
    def forward(self, x):
        # full SST output = latent branch + skip branch
        z = self.encode(x)
        out = self.decode(z)
...
def strip_eval_details_and_write_compact(folder):
    json_files = list(Path(folder).glob("*.json"))
    for path in json_files:
        if path.name.endswith("_compact.json"):
            continue
        ...
        out_path = Path(folder) / "pythia160m_sst_k64_32k_core_compact.json"
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2)

        print(f"[main] wrote compact file: {out_path.name}")
...
def main():
    ...
    if sae.W_skip is not None:
        print(f"[main] W_skip.shape={tuple(sae.W_skip.shape)}")
    else:
        print("[main] W_skip not found on loaded object")

    selected_saes = [("eleuther_pythia160m_sst_k64_32k_layers6mlp", sae)]
    ...
    strip_eval_details_and_write_compact(output_folder)
