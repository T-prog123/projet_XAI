...
def main():
    print("[main] starting ST absorption")
    ...
    config.max_k_value = 1
    config.k_sparse_probe_num_epochs = 1
    config.min_feats_for_eval = 1

    selected_saes = [("eleuther_pythia160m_st_k64_65k_layers6mlp_absorption_fast", sae)]

    print("[main] wrapper ready")
    print(f"[main] device={device}")
    print(f"[main] hook={tl_hook_name}")
    print(f"[main] output_folder={output_folder}")
    print(f"[main] max_k_value={config.max_k_value}")
    print(f"[main] k_sparse_probe_num_epochs={config.k_sparse_probe_num_epochs}")
    print(f"[main] min_feats_for_eval={config.min_feats_for_eval}")
